...
int getRemovableDrives(){
    ...
    while (driveMask != 0)
    {
        if (driveMask & 1)
        {
            ...
            {
                std::cout << qPrintable(QString::fromWCharArray(&drivename[4])) << std::endl;
                removableDisks++;
            }
        }
        driveMask >>= 1;
    ...
    if (removableDisks == 0) {
        std::cout << "No removable disks found" << std::endl;
    }
...
int getDeviceNodePath(QString *volumeLetter){
        ...
        case (0):
            ...
            while(rpibootThread.isRunning());
        }

        QThread::sleep(1);

        cout << "List of removable media:\n";
        if (!getRemovableDrives()) {
            return 1;
        }

        cout << "Please enter letter of selected volume: ";
        QTextStream inputData(stdin);
        volumeLetter->append(inputData.readLine());
        volumeLetter->append(":\\");
