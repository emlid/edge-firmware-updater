...
ExitStatus WindowsStorageDeviceManager::
    _getDriveNumbersRelatedWith(QString const& mountpoint, QVector<int>* const driveNumbers)
{
    ...
    if (volumeHandle == INVALID_HANDLE_VALUE) {
        return ExitStatus(::GetLastError(), mountpoint + ": Getting volume handle failed.");
    }
    ...
    if (!successful) {
       ...
       return ExitStatus(::GetLastError(), mountpoint + ": Getting disk extents failed.");
    }
    ...
    _getRecommendedBlockSize(QString const& diskPath, ulong* const blockSize)
{
    ...
    if (handle == INVALID_HANDLE_VALUE) {
        return ExitStatus(::GetLastError(), diskPath + ": cant open for getting disk info");
    }
    ...
    if (!successful) {
        ::CloseHandle(handle);
        return ExitStatus(::GetLastError(), diskPath + ": cant get disk info");
    }
    ...
    forEachDevices(GUID guid, WindowsStorageDeviceManager::DeviceHandler_t deviceHandler)
{
    ...
    if (deviceInfoSet == INVALID_HANDLE_VALUE) {
        return ExitStatus(::GetLastError(), "Getting device info set failed.", true);
    }
